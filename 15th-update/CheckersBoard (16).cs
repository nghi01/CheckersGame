...
public class CheckersBoard : MonoBehaviour
{
  ...
  //  List<Piece> skipped = new List<Piece>();
    ...
    private Dictionary<Piece, Piece> get_valid_moves(Piece p, int col, int row)
    {
      Dictionary<Piece, Piece> moves = new Dictionary<Piece, Piece>();
      int left = col-1;
      int right = col+1;
      int localrow = row;

      if ((p.isWhite==isWhite) || (p.isKing)) {
        traverse_left(localrow-1, System.Math.Max(localrow-3, -1), -1, p.isWhite, left, skipped);
        traverse_right(localrow-1, System.Math.Max(localrow-3, -1), -1, p.isWhite, right, skipped);
      }
      if ((p.isWhite!=isWhite) || (p.isKing)) {
        traverse_left(localrow+1, System.Math.Min(localrow+3, 8), 1, p.isWhite, left, skipped);
        traverse_right(localrow+1, System.Math.Min(localrow+3, 8), 1, p.isWhite, right, skipped);
      }
      ...
      for (int row = start; row < stop; row+=step) {
        ...
        if (current == null) {
          ...
          if (last.Count>0) {
            ...
            traverse_left(row+step, r, step, isWhite, left-1, skipped=last);
            traverse_right(row+step, r, step, isWhite, left+1, skipped=last);
          }
          break;
      ...
      for (int row = start; row < stop; row+=step) {
        ...
        if (current == null) {
          ...
          if (last.Count>0) {
            ...
            traverse_left(row+step, r, step, isWhite, right-1, skipped=last);
            traverse_right(row+step, r, step, isWhite, right+1, skipped=last);
          }
          break;
