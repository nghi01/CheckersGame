...
public class CheckersBoard : MonoBehaviour
{
  ...
  //  List<Piece> skipped = new List<Piece>();
    ...
    private void addtoDictionary()
    {
      ...
      Piece p = pieces[2, 2];
      //p.isWhite = true;
      Dictionary<(int, int), List<Piece>> moves = get_valid_moves(p, col, row);
      // foreach (KeyValuePair<(int, int), List<Piece>> kvp in moves)
      //   {
      //     textBox3.Text += string.Format("Key = {0}, Value = {1}", kvp.Key, kvp.Value);
      //   }
    //   foreach(KeyValuePair<(int, int), List<Piece>> pair in moves)
    //     {
    //       Debug.Log(pair.Key);
    ...
    private Dictionary<(int, int), List<Piece>> get_valid_moves(Piece p, int col, int row)
    {
      ...
      int right = col+1;
      int localrow = row;
      Debug.Log("variableName: " + left);
      Debug.Log("dictionary " + left);
      Debug.Log(isWhite);
      // Debug.Log(p.isWhite==isWhite);
      // Debug.Log(p.isKing);
      // Debug.Log(isWhite);
      // Debug.Log(15);
      // Debug.Log("variableName: " + left);
      // Debug.Log(left);
      if (((p.isWhite==isWhite) || (p.isKing)) && p!=null) {
